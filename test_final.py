#!/usr/bin/env python3
"""
Final comprehensive test script with unique phone numbers
"""
...
async def test_phone_otp_flow(client, phone_number):
    """Test phone OTP request and verification flow with specific phone"""
    print(f"📱 Testing Phone OTP Authentication Flow ({phone_number})")

    # Step 1: Request phone OTP
    response = await client.post("/onboarding/request-otp", json={"phone": phone_number})
    if response.status_code != 200:
...
async def test_user_setup_flow(client, phone_number):
    """Test user setup flow with authentication"""
    print(f"👤 Testing User Setup Flow ({phone_number})")

    # Step 1: Get a valid token by completing phone OTP flow
    phone_response = await client.post("/onboarding/request-otp", json={"phone": phone_number})
    ...
    setup_response = await client.post("/onboarding/complete-setup", json={
        "username": f"testuser_{phone_number[-4:]}",
        "first_name": "Test",
        "last_name": "User",
...
async def run_tests():
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        # Phone OTP authentication flow (unique phone)
        success, message = await test_phone_otp_flow(client, "+1999888777")
        print(f"   {message}")
        results.append(success)
        ...
        # User setup flow (unique phone)
        success, message = await test_user_setup_flow(client, "+1888777666")
        print(f"   {message}")
        results.append(success)
